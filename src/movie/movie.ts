...
import {
  createDetailsHtmlStructure,
  createGridElement,
  createListElement,
} from "../api/create-html-structure";
...
export function showMovieList(movieListData: MovieListData[]) {
  ...
  const movieListElement =
    currentListMode === MovieLayoutMode.Grid
      ? createGridElement(movieListData)
      : createListElement(movieListData);
...
export async function showMovieDetails(movieId: number) {
  ...
  console.log("showMoviesDetails", currentMovieDetailsData);
}

export async function showSearchMovies(query: string) {
  if (query?.length > 0) {
    const searchData = await fetchSearchMovie(query);
    showMovieList(searchData);
  }
}
